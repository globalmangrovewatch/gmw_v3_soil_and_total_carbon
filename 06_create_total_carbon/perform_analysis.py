...
class PerformAnalysis(PBPTQProcessTool):
    ...
    def do_processing(self, **kwargs):
        ...
        band_defns.append(rsgislib.imagecalc.BandDefn('agb', self.params["agb_img"], 1))

        rsgislib.imagecalc.band_math(self.params["out_c_img"], "soc+(agb*0.451)", 'GTIFF', rsgislib.TYPE_16UINT, band_defns)
        rsgislib.imageutils.pop_img_stats(self.params["out_c_img"], use_no_data=True, no_data_val=0, calc_pyramids=True)

        rsgislib.imagecalc.image_math(self.params["out_c_img"], self.params["out_co2_img"], "b1*3.67", 'GTIFF', rsgislib.TYPE_16UINT)
        rsgislib.imageutils.pop_img_stats(self.params["out_co2_img"], use_no_data=True, no_data_val=0, calc_pyramids=True)

    def required_fields(self, **kwargs):
        return ["soc_img", "agb_img", "gmw_img", "out_c_img", "out_co2_img"]
