...
class PerformAnalysis(PBPTQProcessTool):
    ...
    def do_processing(self, **kwargs):
        ...
        band_defns = list()
        band_defns.append(rsgislib.imagecalc.BandDefn('low', self.params["low_soc_img"], 1))
        band_defns.append(rsgislib.imagecalc.BandDefn('up', self.params["up_soc_img"], 1))

        rsgislib.imagecalc.band_math(self.params["out_img"], "low+up", 'GTIFF', rsgislib.TYPE_16UINT, band_defns)
        rsgislib.imageutils.pop_img_stats(self.params["out_img"], use_no_data=True, no_data_val=0, calc_pyramids=True)

    def required_fields(self, **kwargs):
        return ["low_soc_img", "up_soc_img", "out_img"]
